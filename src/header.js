...
MP3FrameHeader.prototype.nbsamples = function () {
  return this.layer === 1
    ? 12
    : this.layer === 3 && this.flags & MP3FrameHeader.FLAGS.LSF_EXT ? 18 : 36;
};
...
MP3FrameHeader.prototype.decode = function (stream) {
  ...
  if (stream.read (1) === 0) {
    ...
    throw new Error ('UnderflowError'); // LOSTSYNC
  }
  ...
  while (syncing) {
    ...
    if (stream.sync) {
      if (!stream.available (MP3FrameHeader.BUFFER_GUARD)) {
        ...
        throw new Error ('UnderflowError');
      } else if (
      ...
      stream.seek (ptr * 8);
      if (!stream.doSync ()) throw new Error ('UnderflowError');
    ...
    if (header.layer === 1) {
      ...
      var slots_per_frame = header.layer === 3 &&
...
MP3FrameHeader.free_bitrate = function (stream, header) {
  var pad_slot = header.flags & MP3FrameHeader.FLAGS.PADDING ? 1 : 0,
    slots_per_frame = header.layer === 3 &&
      header.flags & MP3FrameHeader.FLAGS.LSF_EXT
      ? 72
      : 144;
  ...
  while (stream.doSync ()) {
    ...
    if (
      ...
      if (header.layer === 1) {
        ...
        rate =
          (header.samplerate * (N - pad_slot + 1) / slots_per_frame / 1000) | 0;
      }
